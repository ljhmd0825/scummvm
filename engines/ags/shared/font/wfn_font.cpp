...
const WFNChar &WFNFont::GetChar(uint16_t code) const {
	if (IsKSX1001(code)) {
		// for Korean KS X1001 Code
		return *_refs_ext[((code >> 8) - 0xB0) * 94 + (code & 0xff) - 0xA1];
	}
...
WFNError WFNFont::ReadExtFntFromFile(Stream *in, const soff_t data_size) {
	...
	// Read offset table
	std::vector<uint32_t> offset_table;	offset_table.resize(char_count);
	in->ReadArrayOfInt32(reinterpret_cast<int32_t *>(&offset_table.front()), char_count);
